...
int access_cache(int address,char type,int current_time){
    ...
    for(int i =0;i<4;i++){
        if(my_cache[index].lines[i].valid_bit==1 && my_cache[index].lines[i].tag==tag){
            ...
            if(type=='W'){
                my_cache[index].lines[i].dirty_bit=1;
    ...
    if(type=='W'){
        ...
        my_cache[index].lines[i].dirty_bit=0;
   ...
   if(my_cache[index].lines[victim_way].dirty_bit==1){
   ...
   if(type=='W'){
    my_cache[index].lines[victim_way].dirty_bit=1;
   }else{
    my_cache[index].lines[victim_way].dirty_bit=0;
   }
...
int main(){
    ...
    for(int i =0;i<4096;i++){
        for(int j=0;j<4;j++){
            ...
            my_cache[i].lines[j].dirty_bit=0;
        }
    }
